...
class Checker(object):
    ...
    def handleContainer (self, node):
        if isinstance(self.scope, ClassScope) and ((isinstance(node, ast.List) and len(node.elts) == 0) or (isinstance(node, ast.Dict) and len(node.keys) == 0)) and hasattr(node.parent, "targets"):
